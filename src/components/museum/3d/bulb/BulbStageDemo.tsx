...
import { BULB_PARTS, type BulbPartId } from "@/lib/bulb-types";

/**
 * Wrapper demo cho BulbScene — quản lý state (play/pause, speed, annotations,
 * part selection) và render control panel. Dùng trong ExhibitModal khi
 * exhibit.motif === "light-bulb".
 */
export function BulbStageDemo({ height = 320 }: { height?: number }) {
  // Pause + disable auto-rotate when reduced motion — lazy init to avoid setState-in-effect
  const reducedMotion =
    typeof window !== "undefined" && window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  const [playing, setPlaying] = useState(!reducedMotion);
  const [speed, setSpeed] = useState(1);
  const [selectedPart, setSelectedPart] = useState<BulbPartId | null>(null);
  const [showAnnotations, setShowAnnotations] = useState(true);
  const [autoRotate, setAutoRotate] = useState(!reducedMotion);
  const [resetSignal, setResetSignal] = useState(0);

  const selectedPartData = BULB_PARTS.find((p) => p.id === selectedPart);
    ...
    <div className="relative">
      ...
      <div className="absolute left-3 top-3 z-10 flex flex-wrap items-center gap-1.5">
        ...
        <button
          ...
          style={{
            borderColor: showAnnotations ? "#e8b53a88" : "oklch(0.5 0.02 60 / 0.22)",
            color: showAnnotations ? "#e8b53a" : "oklch(0.5 0.02 60 / 0.8)",
          }}
        >
        ...
        <button
          ...
          className="inline-flex h-7 items-center gap-1 rounded-full border border-foreground/20 bg-background/70 px-2 text-[0.65rem] text-foreground/80 backdrop-blur-sm transition hover:border-foreground/40 hover:text-foreground"
          style={{
            borderColor: autoRotate ? "#e8b53a88" : "oklch(0.5 0.02 60 / 0.22)",
            color: autoRotate ? "#e8b53a" : "oklch(0.5 0.02 60 / 0.8)",
          }}
        >
      ...
      {selectedPartData && (
        <div className="absolute bottom-3 left-3 right-3 z-10 rounded-lg border border-amber-400/30 bg-background/85 p-2.5 backdrop-blur-md">
          <div className="flex items-center gap-2">
            <span className="h-1.5 w-1.5 rounded-full bg-amber-400" />
            <span className="text-[0.7rem] font-semibold text-amber-300">{selectedPartData.label}</span>
            <button
