...
import { NEURAL_PARTS, type NeuralPartId } from "@/lib/neural-types";

/**
 * Wrapper demo cho NeuralScene — quản lý state (play/pause, speed, annotations,
 * part selection, autoRotate, reset) và render control panel.
 * Dùng cho Phase 4 hero exhibit "neural-net" (AlexNet).
 * Pink accent (Phase 4) cho control bar.
 */
export function NeuralStageDemo({ height = 320 }: { height?: number }) {
  // Pause khi reduced motion — lazy init tránh setState-in-effect
  const reducedMotion =
    typeof window !== "undefined" && window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  const [playing, setPlaying] = useState(!reducedMotion);
  const [speed, setSpeed] = useState(1);
  const [selectedPart, setSelectedPart] = useState<NeuralPartId | null>(null);
  const [showAnnotations, setShowAnnotations] = useState(true);
  const [autoRotate, setAutoRotate] = useState(!reducedMotion);
  const [resetSignal, setResetSignal] = useState(0);

  const selectedPartData = NEURAL_PARTS.find((p) => p.id === selectedPart);

  // Pink accent (Phase 4)
  const ACCENT = "#e879f9";
  const ACCENT_DIM = "#a21caf";
    ...
    <div className="relative">
      ...
      <div className="absolute left-3 top-3 z-10 flex flex-wrap items-center gap-1.5">
        ...
        <button
          ...
          style={{
            borderColor: showAnnotations ? `${ACCENT}88` : "oklch(0.5 0.02 60 / 0.22)",
            color: showAnnotations ? ACCENT : "oklch(0.5 0.02 60 / 0.8)",
          }}
        >
        ...
        <button
          ...
          className="inline-flex h-7 items-center gap-1 rounded-full border border-foreground/20 bg-background/70 px-2 text-[0.65rem] text-foreground/80 backdrop-blur-sm transition hover:border-foreground/40 hover:text-foreground"
          style={{
            borderColor: autoRotate ? `${ACCENT}88` : "oklch(0.5 0.02 60 / 0.22)",
            color: autoRotate ? ACCENT : "oklch(0.5 0.02 60 / 0.8)",
          }}
        >
      ...
      <div className="absolute right-3 top-3 z-10 flex items-center gap-1.5 rounded-full border border-foreground/20 bg-background/70 px-2.5 py-1 backdrop-blur-sm">
        ...
        <input
          ...
          className="h-1 w-16 cursor-pointer accent-fuchsia-500"
        />
      ...
      {selectedPartData && (
        <div
          className="absolute bottom-3 left-3 right-3 z-10 rounded-lg border bg-background/85 p-2.5 backdrop-blur-md"
        ...
        >
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-[0.7rem] font-semibold" style={{ color: ACCENT }}>
          ...
          <div
            className="mt-1.5 text-[0.55rem] uppercase tracking-[0.16em]"
