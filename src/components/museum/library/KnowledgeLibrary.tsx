...
import { Library, ArrowLeft, ArrowRight, BookOpen, Clock, Sparkles, Flame, Zap, Cpu, Brain, Award, GraduationCap, ListChecks, BookMarked } from "lucide-react";
import { useMuseum } from "@/lib/store";
import { BrandMark } from "@/components/museum/layout/brand";
import { CHAPTERS } from "@/lib/knowledge-data";
import { TOTAL_KNOWLEDGE_QUESTIONS } from "@/lib/knowledge-quiz";
      ...
      <div className="relative z-10 flex min-h-screen flex-col">
        ...
        <main className="mx-auto flex w-full max-w-5xl flex-1 flex-col justify-center px-4 py-10 sm:px-6">
          ...
          >
            {CHAPTERS.map((ch, i) => (
              <motion.button
                ...
                onClick={() => {
                  setCurrentLessonId(ch.lessons[0]?.id ?? null);
                ...
                style={{
                  borderColor: `${ch.accent}33`,
                  background: `${ch.accent}08`,
                }}
              >
                <div className="absolute inset-x-0 top-0 h-px" style={{ background: `linear-gradient(90deg, transparent, ${ch.accent}, transparent)` }} />
                <div className="absolute inset-0 opacity-0 transition-opacity group-hover:opacity-100" style={{ background: `radial-gradient(ellipse at top, ${ch.accent}10, transparent 70%)` }} />
                <div className="relative">
                  <div className="flex items-center gap-2">
                    <GraduationCap className="h-4 w-4" style={{ color: ch.accent }} />
                    <span className="text-[0.6rem] uppercase tracking-[0.15em]" style={{ color: ch.accent }}>Chương {ch.index}</span>
                  </div>
                  <h3 className="mt-2 font-serif text-sm font-bold leading-tight text-foreground/90">{ch.title}</h3>
                  <p className="mt-1 text-xs text-foreground/55">{ch.subtitle}</p>
                  <div className="mt-3 flex items-center gap-2 text-[0.6rem] text-foreground/45">
          ...
          <motion.button
            ...
            className="group relative mx-auto mt-6 w-full max-w-3xl overflow-hidden rounded-2xl border p-5 text-left transition-all hover:-translate-y-1"
            style={{ borderColor: "rgba(232,180,58,0.25)", background: "linear-gradient(135deg, rgba(232,180,58,0.05) 0%, oklch(0.16 0.012 60) 100%)" }}
          >
            <div className="absolute inset-x-0 top-0 h-px" style={{ background: "linear-gradient(90deg, transparent, #e8b53a, transparent)" }} />
            <div className="relative flex items-center gap-4">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-[0.6rem] uppercase tracking-[0.15em] text-foreground/50">Kiểm tra kiến thức</div>
                <h3 className="mt-0.5 font-serif text-lg font-bold text-foreground">Trắc nghiệm tổng hợp</h3>
                <p className="mt-0.5 text-xs text-foreground/55">{TOTAL_KNOWLEDGE_QUESTIONS} câu hỏi · CMCN, CNH-HĐH, Hội nhập</p>
              </div>
              <div className="flex items-center gap-2">
                {knowledgeQuizScore !== null && (
                  <span className="inline-flex items-center gap-1 rounded-full border border-amber-400/30 bg-amber-400/10 px-2 py-1 text-xs font-semibold text-amber-300">
                    <Award className="h-3 w-3" />{knowledgeQuizScore}/{TOTAL_KNOWLEDGE_QUESTIONS}
                  </span>
                )}
