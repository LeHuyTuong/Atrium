 ...
 *
 *  Pass isSimMaster={false} when embedding in Scene Lab to avoid running
 *  physics simulation twice (the background canvas already runs it). */
export function EngineModel({ isSimMaster = true }: { isSimMaster?: boolean }) {
  ...
  useFrame((_, dtRaw) => {
    if (!isSimMaster) {
      // Scene Lab: just advance the clock visuals, don't touch store physics
      const store = useEngineStore.getState();
      advanceClock(store.actualRpm > 0 ? store.actualRpm : 18, Math.min(dtRaw, 0.05));
      return;
    }
